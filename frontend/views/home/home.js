...
function renderResults(results) {
  ...
  results.forEach((item) => {
    ...
    div.className =
      "p-3 cursor-pointer hover:bg-gray-100 transition duration-150";
    div.innerHTML = `
...
async function loadFeaturedEstablishments() {
  try {
    ...
    Alerts.showError(
      "Error de Carga",
      "No se pudieron cargar los establecimientos destacados."
    );
  }
}
...
function renderGallery(items) {
  ...
  items.forEach((item) => {
    ...
    card.className =
      "bg-white rounded-xl shadow-lg hover:shadow-2xl transition duration-300 overflow-hidden cursor-pointer";
